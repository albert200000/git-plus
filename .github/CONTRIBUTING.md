...
* Most commands are defined as separate modules in `lib/models`.
  ...
  * `::cmd` usually only needs two arguments. The first is the array of command line arguments to be used with git e.g. `['status', '--porcelain']` and an options object with a `cwd` key and value of the path to the repository. Here is a good [example](https://github.com/albert200000/git-plus/blob/master/lib/git.coffee#L112L116).
