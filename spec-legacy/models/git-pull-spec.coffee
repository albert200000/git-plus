...
describe "Git Pull", ->
  ...
  describe "when 'promptForBranch' is enabled", ->
    it "calls git.cmd with ['remote']", ->
      atom.config.set('pulsar-git-plus.remoteInteractions.promptForBranch', true)
      GitPull(repo)
      expect(git.cmd).toHaveBeenCalledWith ['remote'], options

  describe "when 'pullRebase' is enabled", ->
    it 'calls git.cmd with --rebase', ->
      atom.config.set('pulsar-git-plus.remoteInteractions.pullRebase', true)
      GitPull(repo)
  ...
  describe "when 'pullAutostash' is enabled", ->
    it 'calls git.cmd with --autostash', ->
      atom.config.set('pulsar-git-plus.remoteInteractions.pullAutostash', true)
      GitPull(repo)
