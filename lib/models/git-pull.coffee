...
module.exports = (repo) ->
  extraArgs = if atom.config.get('pulsar-git-plus.remoteInteractions.pullRebase') then ['--rebase'] else []
  if atom.config.get('pulsar-git-plus.remoteInteractions.pullAutostash')
