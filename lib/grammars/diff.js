(function() {
  ...
  function runCodeGrammar(grammar, tags, tokens, tokenTagIndices, codeStack) {
    ...
    for (let i = tokens.length - 1; i >= 0; i--) {
      ...
      if (codeStack) {
        for (let j = 0; j < codeStack.length; j++) {
          ...
          if (scope) {
            tagStack.push(
              (atom.grammars.textmateRegistry || atom.grammars.registry).startIdForScope(scope)
            )
          }
        }
      }
    ...
    codeStack =
      (addedResult && addedResult.ruleStack) ||
      (removedResult && removedResult.ruleStack) ||
      codeStack ||
      null
